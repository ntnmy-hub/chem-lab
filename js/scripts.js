...
const reactions = {
  ...
  "CuSO4+Ba(OH)2": { text: "Cu(OH)2 + BaSO4", color: "#66ccff" },
...
function start() {
  ...
  setTimeout(() => {
    document.getElementById("lab").style.opacity = "1";
...
function select(el, chem) {
  ...
  if (selected.length === 2) {
    setTimeout(() => { react(selected[0], selected[1]); reset(); }, 500);
  }
}
...
function checkIndicator(a, b) {
  ...
  if (a === "Quỳ tím" || b === "Quỳ tím") {
    if (acids.includes(a) || acids.includes(b)) { liquid.style.background = "#ff6b81"; return "Quỳ tím → đỏ (acid)"; }
    if (bases.includes(a) || bases.includes(b)) { liquid.style.background = "#18455a"; return "Quỳ tím → xanh (base)"; }
    return "Không đổi màu";
  }
  if (a === "Phenolphtalein" || b === "Phenolphtalein") {
    if (bases.includes(a) || bases.includes(b)) { liquid.style.background = "pink"; return "Hóa hồng (base)"; }
    return "Không màu";
  }
  return null;
}
...
function react(a, b) {
  ...
  if (indicator) { document.getElementById("result").innerText = "Kết quả: " + indicator; return; }

  if ((a === "Cu" && acids.includes(b)) || (b === "Cu" && acids.includes(a))) {
    document.getElementById("result").innerText = "Kết quả: Cu không phản ứng"; return;
  }
  ...
  if (reaction) {
    ...
    if (reaction.color) liquid.style.background = reaction.color;
    if (reaction.gas) showGasEffect();
    showEffect();
  } else {
...
function showEffect() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    bubble.style.left = Math.random() * 25 + 35 + "%"; 
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const svgFileName = "binhtamgiac.svg"; 

  flasks.forEach(flask => {
    flask.insertAdjacentHTML('afterbegin', `
      <div class="flask-liquid"></div>
      <img src="${svgFileName}" class="flask-svg" alt="Bình chứa">
    `);
  });
});